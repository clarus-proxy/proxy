...
public class AuthenticationHandler extends PgsqlMessageHandler<PgsqlAuthenticationMessage> {
    ...
    @Override
    protected PgsqlAuthenticationMessage process(ChannelHandlerContext ctx, PgsqlAuthenticationMessage msg)
            throws IOException, NoSuchAlgorithmException {
        if (msg instanceof PgsqlStartupMessage) {
            PgsqlStartupMessage msgStartup = (PgsqlStartupMessage) msg;
            LOGGER.debug("User id: {}, database: {}", msgStartup.getParameters().get(CString.valueOf("user")),
                    msgStartup.getParameters().get(CString.valueOf("database")));
            PgsqlStartupMessage newMsg = msgStartup;
            CString userIdentificate = getEventProcessor(ctx).processUserAuthentication(ctx,
                    msgStartup.getParameters());
            ...
            case AUTHENTICATION_MD5_PASSWORD:
                ...
                int newAuthenticationType = getEventProcessor(ctx).processAuthenticationParameters(ctx,
                        authenticationType, newAuthenticationMessage.getAuthenticationParameters());
