package eu.clarussecure.proxy.protocol.plugins.http.handler.decoder;
...
public class HttpObjectAccumulator extends MessageAccumulator<HttpObject, HttpMessage, HttpContent, FullHttpMessage> {

    private static final Logger LOGGER = LoggerFactory.getLogger(HttpObjectAccumulator.class);
     ...
     *            will be called.
     */
    public HttpObjectAccumulator(int maxContentLength) {
     ...
     */
    public HttpObjectAccumulator(int maxContentLength, boolean closeOnExpectationFailed) {
    ...
    protected void handleOversizedMessage(final ChannelHandlerContext ctx, HttpMessage oversized) throws Exception {
        if (oversized instanceof HttpRequest) {
            ...
            ChannelFuture future = ctx.writeAndFlush(TOO_LARGE.duplicate().retain())
